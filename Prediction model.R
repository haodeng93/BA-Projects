#STAT630 Project
#Step 1: filled missing data with missForest
data=read.csv("formymanHao.csv")
...
## the ones without a disease status -don't know if they have CKD
out_sample=which(is.na(data$CKD)==1)
out_sample
data_out=data[out_sample,]  
data_out
write.csv(data_out, "Processed data.csv")

## the ones with a disease status
data_in=data[-out_sample,]  
data_in
write.csv(data_in, "Processed data disease.csv")

summary(data_in)
data_in$Age <- as.integer(data_in$Age)
data_in$Female <- as.factor(data_in$Female)
data_in$Educ <- as.factor(data_in$Educ)
data_in$Unmarried <- as.factor(data_in$Unmarried)
data_in$Insured <- as.factor(data_in$Insured)
data_in$Income <- as.factor(data_in$Income)
data_in$Obese <- as.factor(data_in$Obese)
data_in$Dyslipidemia <- as.factor(data_in$Dyslipidemia)
data_in$PVD <- as.factor(data_in$PVD)
data_in$Activity <- as.factor(data_in$Activity)
data_in$PoorVision <- as.factor(data_in$PoorVision)
data_in$Smoker <- as.factor(data_in$Smoker)
data_in$Hypertension <- as.factor(data_in$Hypertension)
data_in$Fam.Hypertension <- as.factor(data_in$Fam.Hypertension)
data_in$Diabetes <- as.factor(data_in$Diabetes)
data_in$Fam.Diabetes <- as.factor(data_in$Fam.Diabetes)
data_in$Stroke <- as.factor(data_in$Stroke)
data_in$CVD <- as.factor(data_in$CVD)
data_in$Fam.CVD <- as.factor(data_in$Fam.CVD)
data_in$CHF <- as.factor(data_in$CHF)
data_in$Anemia <- as.factor(data_in$Anemia)
data_in$CKD <- as.factor(data_in$CKD)
#there is no missing value for Racegrp and CareSource

######-------------- Step 2  - apply missForest method --------------######
#install.packages("missForest")
library(missForest)

#seed 10% missing values
data_in.mis <- prodNA(data_in, noNA = 0.1)
summary(data_in.mis)
#weird: this shows missing data for Racegrp and CareSource

#impute missing values, using all parameters as default values
data_in.imp <- missForest(data_in.mis)

#check imputed values
data_in.imp$ximp

#check imputation error
data_in.imp$OOBerror

#comparing actual data accuracy
data_in.err <- mixError(data_in.imp$ximp, data_in.mis, data_in)
data_in.err

write.csv(data_in.imp[["ximp"]], 'missforest result.csv')

#predict model
setwd("~/Desktop/MSBA/STAT630/Group Project")
data=read.csv("missforest result copy.csv")
names(data)
...
class(data)
summary(data)
#remove first 1000 ckd row for test 
#out_sample=which(is.na(data$CKD)==1)
#data_out=data[out_sample,]   ## the ones without a disease status
data_in=data   ## the ones with a disease status
summary(data_in)
...
#drop research based unrelated variables
names(data)
names(data_in)
#V1
modeltest=glm(CKD~Age+CHF+PVD+DBP+Activity+Hypertension+Fam.Hypertension+Diabetes+CVD+Fam.CVD+Anemia,family="binomial",data=data_in)
#V2: modeltest=glm(CKD~Age+Female+PVD+DBP+Activity+Hypertension+Fam.Hypertension+Diabetes+Stroke+CVD+Fam.CVD+Anemia+CHF,family="binomial",data=data_in)
summary(modeltest)
model4=step(modeltest,direction="backward")
summary(model4) #only female got removed from the formula
#model2=step(model,direction="forward") #if you don't care what is in the data, remove the lowest p at a time
#summary(model2)
model3=step(model,direction="backward") #starts all variables and remove the highest p vlaue
...
newdata1=data_out[1:1000,]  ## these are 4 "new patients"
...
summary(phat3)
hist(phat3)
classify=ifelse(phatnew>.2,1,0)  # this is a threshold, we say if probability >50% , then say "yes"
...
modeltest=glm(CKD~Age+PVD+DBP+Activity+Hypertension+Fam.Hypertension+Diabetes+CVD+Fam.CVD+Anemia,family="binomial",data=data_in)
#V2: modeltest=glm(CKD~Age+Female+PVD+DBP+Activity+Hypertension+Fam.Hypertension+Diabetes+Stroke+CVD+Fam.CVD+Anemia+CHF,family="binomial",data=data_in)
summary(modeltest)
model4=step(modeltest,direction="backward")
summary(model4) 
newdata2=data_in  ## these are 4 "new patients"
newdata2
phatnew1=predict(model4, newdata = newdata2, type = "response")
phatnew1
classify=ifelse(phatnew1>.2,1,0)
c_accuracy(data_in$CKD,classify)
formula(model4)
summary(model4)
write.csv(phatnew1,'phatnew1-6000 V3.csv')
hist(phatnew1)
